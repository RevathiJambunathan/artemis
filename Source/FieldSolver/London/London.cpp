...
London::EvolveLondonJ (amrex::Real dt)
{
    ...
    amrex::ParallelFor(tjx, tjy, tjz,
        [=] AMREX_GPU_DEVICE (int i, int j, int k) {
            if (sc_arr(i,j,k)==1 and sc_arr(i+1,j,k)==1) {
                ...
                if ( i == 16 and j == 16 and k == 81) {
                  amrex::Print() << dt << "(i,j,k) : " << i << "," << j << "," << k << " jy " << jx_arr(i,j,k) << " Ey " << Ex_arr(i,j,k) << " lambda_sq inv " << lambda_sq_inv << " mu interp : " << mu_interp << " " <<  "\n";
                }
            }
        },
        [=] AMREX_GPU_DEVICE (int i, int j, int k) {
        ...
        const int j = 16;
        const int k = 81;
        amrex::AllPrintToFile("FieldOutput") << warpx.getistep(0) << " " << warpx.gett_new(0) << " " << dt << "(i,j,k) : " << i << "," << j << "," << k << " jx " << jx_arr(i,j,k) << " Ex " << Ex_arr(i,j,k) << "\n";
    }
}
